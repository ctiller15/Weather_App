...
class WeatherApp extends React.Component {
	...
	componentDidMount() {
		// IIFE with this bound to the enclosing scope.
		(() =>  {
			if ("geolocation" in navigator) {
		  		navigator.geolocation.getCurrentPosition((position) => {
		  			this.populateWeatherData(position.coords.latitude, position.coords.longitude);

		  		}, (err) => {
		  			this.locationFallback();
		  		});
			} else {
		  		// if the user doesn't have geolocation in the browser, provide a fallback.
		  		this.locationFallback();
			}
		})();	
	}
	...
	populateWeatherData(lat, lon) {
		...
		Promise.all([
			...
			fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=c753b4feae5388aca46414f0ab6a4c14`).then(function(response){
				return response.json();
			}),
			fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=c753b4feae5388aca46414f0ab6a4c14`).then(function(response){
				return response.json();
			})
	...
	// Used if geolocation was denied or does not work
	locationFallback(){
		fetch(`http://ip-api.com/json`).then(function(response){
			return response.json();
		}).then((data) => {
			this.populateWeatherData(data.lat, data.lon);
		});
	}
